...
function cargarEventListeners(){
	catalogos.addEventListeners(´click´, comprarCatalogo);
	carrito.addEventListeners(´click´ eliminarCatalogo);
...
function insertarCarrito(catalogo){
	...
	row.innerHTML = `
         <td>
            <img scr="${catalogo.imagen}" width=100>
         </td>
         <td>${catalogo.titulo}<td>
         <td>${catalogo.precio}<td>
        <td>
            <a href="#" class="borrar-catalogo" data-id="${catalogo.id}"></a>
        <td>
	`;
	listaCatalogos.appendChild(row);
	guardarCatalogosLocalStorage(catalogo);
}
...
function guardarCatalogoLocalSrorage(catalogo) {
	let catalogos;

	catalogos = obtenerCatalogoLocalStorage();
...
function obtenerCatalogoLocalStorage() {
	let catalogosLS;
...
function leerLocalStorage() {
	...
	catalogosLS = obtenerCatalogoLocalStorage();
